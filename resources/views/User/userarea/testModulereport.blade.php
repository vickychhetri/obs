...
<div class="blank-page-content">
    ...
    <div class="outer-w3-agile mt-3">
        ...
        <h2> <b style="font-weight: bold;"> Report Test Name: </b> Module - {{$TestReportDataset['TestName']}} </h2>
        ...
        <div class="container">
                    <center>
                        <h1> Detail Question Report </h1>
                        <p> [Red Color: Wrong Answer, Green Color: Correct Answer]</p>
                    </center>
                    <div class="row">
                        <?php
                $count=1; ?>
                        ...
                        <div class="col-md-12">
                            <div
                                style="border:1px solid black;margin:10px;padding:16px;background-color:{{$questionBank['AnswerCorrect']==0? '#ffd6d6':'#e8ffe6'}}">
                                <h2>{{$count++}}). {{$questionBank['Question']}}</h2>
                                <h3 style="color:{{$questionBank['AnswerCorrect']==0? 'red':'green'}}">
                                    Selected Answer :
                                    <?php 
                        if($TestReportDataset['TestName']=="1"){
                            ?>
                                    {{$questionBank['SelectedAnswer']=="1"?'Acyanotic':'Cyanotic'}}
                                    <?php
                        }else  if($TestReportDataset['TestName']=="5") {
                            ?>
                                    {{$questionBank['SelectedAnswer']=="1"?'TRUE':'FALSE'}}
                                    <?php
                        }
                        ?>

                                </h3>
                                <h3> Is this answer correct: {{$questionBank['AnswerCorrect']==1? 'YES':'NO'}}</h3>
                            </div>
                        </div>
                        @endforeach
                    </div>
